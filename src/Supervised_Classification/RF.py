# -*- coding: utf-8 -*-
"""
Elliptic node classification with RandomForest (refactored)

- Lädt vorbereitete Artefakte (Parquet) aus ../artifacts/elliptic/<ARTIFACT_DIR_NAME>
- Keine Feature Selection
- Kein Feature-Scaling (RF braucht kein Scaling)
- Oversampling der Minoritätsklasse (Fraud=0) auf 1:1
- Deterministische Hyperopt (np.random.default_rng)
- Ergebnisse als JSON gespeichert unter ../Results_JSON/elliptic_RF_<ARTIFACT_DIR_NAME>.json
"""
...
RESULTS_DIR = EXPERIMENTS_ROOT / "Results_HyPa" / "RF"
...
def apply_balancing_strategy(X, y, params):
    ...
    if strategy == "none":
        ...
        # Random Forest unterstützt class_weight
        return X, y, "balanced"
...
def calculate_rf_intrinsic_importance(clf, X, y):
    """
    Berechnet intrinsische Feature Importance für Random Forest basierend auf
    den integrierten Gini-Importance-Werten.
    """
    # Random Forest bietet bereits Feature Importance (Gini Importance)
...
params_space = {
    "n_estimators": hp.quniform(
        "n_estimators", 100, 600, 25
    ),  # 100..600 in 25er-Schritten
    "max_depth": hp.choice("max_depth", ["None"] + [5, 8, 12, 16, 20, 30, 40]),
    ...
    "balancing_strategy": hp.choice(
        ...
        [
            "none",
            "oversampling",
            "class_weights",  # RF unterstützt class_weight!
        ],
    ),
}
...
def run_for_artifact(
    ...
    # Sicherheit: in int konvertieren (0/1)
    ...
    def objective(params):
        """
        Trainiert RF und wertet auf Validation aus.
        Minimiert LogLoss.
        """
        ...
        # Metriken (Fraud=0 als positive Klasse)
    ...
    # A) TRAIN+VAL zusammenbauen
    Xtv = np.vstack([Xtr, Xva])  # Roh-Features, kein Scaler nötig
    ytv = np.concatenate([y_train.values, y_val.values])

    # B) Balancing-Strategie auf TRAIN+VAL anwenden
    Xtv_bal, ytv_bal, class_weights_tv = apply_balancing_strategy(
        Xtv, ytv, best_params
    )

    # C) max_features korrekt mappen
    max_features_val = best_params["max_features"]
    if max_features_val == "None":
        max_features_val = None

    # D) Finalen RF auf TRAIN+VAL fitten
    clf = RandomForestClassifier(
        ...
        max_features=max_features_val,
        class_weight=class_weights_tv,  # <- aus TRAIN+VAL
        bootstrap=True,
        n_jobs=-1,
        random_state=42,
    )
    ...
    if COMPUTE_FEATURE_IMPORTANCE:
        ...
        print("Calculating intrinsic Random Forest importance...")
        feature_importance_intrinsic = calculate_rf_intrinsic_importance(
            clf, Xtv_bal, ytv_bal
        )
        calculation_method_intrinsic = "rf_gini_importance"
    ...
    try:
        ...
        preds_path = os.path.join(
            results_dir, f"elliptic_RF_{artifact_dir_name}__testpreds.parquet"
        )
    ...
    # JSON-EXPORT (mit robustem Serializer – Fix für int32 usw.)
    summary = {
        "run_info": {
        ...
        "feature_importance": {
            ...
            "intrinsic_importance": {
                ...
                "importance_type": "gini",  # RF standard importance type
            },
        },
    }

    json_path = os.path.join(
        RESULTS_DIR, f"elliptic_RF_{artifact_dir_name}.json"
    )
    ...
    df_perm_path = out_base / f"elliptic_RF_{artifact_tag}__perm.parquet"
    df_intr_path = out_base / f"elliptic_RF_{artifact_tag}__intr.parquet"
    ...
    trials_path = out_base / f"elliptic_RF_{artifact_tag}__trials.joblib"
    ...
    bundle_path = out_base / f"elliptic_RF_{artifact_tag}__bundle.joblib"
...
def main():
    ap = argparse.ArgumentParser(
        description="RandomForest Elliptic – Single & Batch Runner"
    )
    ap.add_argument(
        "--artifact",
    ...
    for name in sorted(os.listdir(root)):
        ...
        if args.skip_existing:
            out = os.path.join(RESULTS_DIR, f"elliptic_RF_{name}.json")
