...
- Evaluation: identical to GCN/GAT via evaluate(), including validation threshold tuning τ* (THRESH_METRIC)

Key points:
- Hyperopt searches over:
  * Pretraining parameters (hidden_dim, learning_rate_pretrain, edge_drop_prob)
  * Classifier parameters (learning rate, weight_decay, class_weight_base/exp)
...
- Pretraining is:
  * run for a shorter duration per Hyperopt trial
...
class GCLClassifier(nn.Module):
    ...
    def forward(self, g, x, return_embeddings: bool = False):
        ...
        if return_embeddings:
            return logits, z  # (logits, embeddings)
        return logits


def export_gcl_embeddings_to_parquet(
    classifier,
    g,
    features,
    tx_ids,
    ...
    """
    Exportiert Node-Embeddings eines trainierten GCL-Classifiers als Parquet-Datei.

    - Datei:   ROOT_DIR/artifacts/gcl_embeddings_<artifact>[_<drop_rate>_<seed>].parquet
    - Spalten: txId, gcl_emb_0, gcl_emb_1, ...
    """

    classifier.eval()
    with th.no_grad():
        # logits brauchen wir hier eigentlich nicht – wir wollen die Embeddings z
        logits, z = classifier(g, features, return_embeddings=True)

    z_np = z.detach().cpu().numpy()

    # tx_ids in saubere numpy-Strings konvertieren
    ...
    dim = z_np.shape[1]
    emb_cols = [f"gcl_emb_{i}" for i in range(dim)]

    df = pd.DataFrame(z_np, columns=emb_cols)
    df.insert(0, "txId", tx_ids_np)

    global_artifact_dir = os.path.join(ROOT_DIR, "artifacts")
    os.makedirs(global_artifact_dir, exist_ok=True)

    suffix, _ = GCN.get_edge_suffix_and_path(
        drop_rate=drop_rate, variant=variant
    )
    fname = f"gcl_embeddings{suffix}.parquet"
    out_path = os.path.join(global_artifact_dir, fname)

    df.to_parquet(out_path, index=False)
    print(f"GCL-Embeddings als Parquet gespeichert: {out_path}")
...
def run_for_artifact(
    ...
    try:
        ...
        json_path = os.path.join(
            results_dir, f"elliptic_GCL_{artifact_dir_name}{suffix}.json"
        )
    ...
    export_gcl_embeddings_to_parquet(
        classifier=clf,
        g=g,
    ...
    validation_metrics = {
        "at_0_5": {
            "f1_fraud": float(val_results_05["f1_score"]),
            "precision_fraud": float(val_results_05["precision"]),
            "recall_fraud": float(val_results_05["recall"]),
            "balanced_accuracy": float(val_results_05["balanced_accuracy"]),
            "log_loss": float(val_results_05["log_loss"]),
            "accuracy": float(val_results_05["accuracy"]),
            "roc_auc_fraud": float(val_results_05["roc_auc"]),
            "pr_auc_fraud": float(val_results_05["pr_auc"]),
            "pred_fraud": int(val_results_05["indices_fraud"]),
            "pred_no_fraud": int(val_results_05["indices_no_fraud"]),
            "actual_fraud": int(val_results_05["labels_fraud"]),
            "actual_no_fraud": int(val_results_05["labels_no_fraud"]),
            "confusion_matrix": val_results_05["confusion_matrix"],
        },
        "at_tau_star": {
            "tuned_threshold": float(tau_star),
            "metric": THRESH_METRIC,
            "f1_fraud": float(val_results_tuned["f1_score"]),
            "precision_fraud": float(val_results_tuned["precision"]),
            "recall_fraud": float(val_results_tuned["recall"]),
            "balanced_accuracy": float(val_results_tuned["balanced_accuracy"]),
            "log_loss": float(val_results_tuned["log_loss"]),
            "accuracy": float(val_results_tuned["accuracy"]),
            "roc_auc_fraud": float(val_results_tuned["roc_auc"]),
            "pr_auc_fraud": float(val_results_tuned["pr_auc"]),
            "pred_fraud": int(val_results_tuned["indices_fraud"]),
            "pred_no_fraud": int(val_results_tuned["indices_no_fraud"]),
            "actual_fraud": int(val_results_tuned["labels_fraud"]),
            "actual_no_fraud": int(val_results_tuned["labels_no_fraud"]),
            "confusion_matrix": val_results_tuned["confusion_matrix"],
        },
    }

    final_test_metrics = {
        "at_0_5": {
            "f1_fraud": float(test_results_05["f1_score"]),
            "precision_fraud": float(test_results_05["precision"]),
            "recall_fraud": float(test_results_05["recall"]),
            "balanced_accuracy": float(test_results_05["balanced_accuracy"]),
            "log_loss": float(test_results_05["log_loss"]),
            "accuracy": float(test_results_05["accuracy"]),
            "roc_auc_fraud": float(test_results_05["roc_auc"]),
            "pr_auc_fraud": float(test_results_05["pr_auc"]),
            "pred_fraud": int(test_results_05["indices_fraud"]),
            "pred_no_fraud": int(test_results_05["indices_no_fraud"]),
            "actual_fraud": int(test_results_05["labels_fraud"]),
            "actual_no_fraud": int(test_results_05["labels_no_fraud"]),
            "confusion_matrix": test_results_05["confusion_matrix"],
        },
        "at_tau_star": {
            "tuned_threshold": float(tau_star),
            "metric": THRESH_METRIC,
            "f1_fraud": float(test_results_tuned["f1_score"]),
            "precision_fraud": float(test_results_tuned["precision"]),
            "recall_fraud": float(test_results_tuned["recall"]),
            "balanced_accuracy": float(
                test_results_tuned["balanced_accuracy"]
            ),
            "log_loss": float(test_results_tuned["log_loss"]),
            "accuracy": float(test_results_tuned["accuracy"]),
            "roc_auc_fraud": float(test_results_tuned["roc_auc"]),
            "pr_auc_fraud": float(test_results_tuned["pr_auc"]),
            "pred_fraud": int(test_results_tuned["indices_fraud"]),
            "pred_no_fraud": int(test_results_tuned["indices_no_fraud"]),
            "actual_fraud": int(test_results_tuned["labels_fraud"]),
            "actual_no_fraud": int(test_results_tuned["labels_no_fraud"]),
    ...
    out = {
        ...
        "tuned_threshold": float(tau_star),
        # NEU: GCN/GAT-kompatible Blöcke
        "validation_metrics": validation_metrics,
        "final_test_metrics": final_test_metrics,
        # ALT: Deine bisherigen Felder bleiben zur Sicherheit bestehen
...
# CLI / Batch-Runner (analog GCN/GAT)
def main():
    ...
    if args.skip_existing:
        ...
        for name in candidates:
            ...
            out_path = os.path.join(
                RESULTS_DIR, f"elliptic_GCL_{name}{suffix}.json"
            )
