...
- Retained scaling fix (only labeled training rows are scaled)

Patched (NaN / overflow hardening):
- Numerically safe evaluate(): nan_to_num, row normalization, clipping, fallback log loss
...
- Softmax / logits are sanitized before and after softmax
"""
...
class Net1(nn.Module):
    ...
    def forward(self, g, x, return_embeddings: bool = False):
        x = self.layer1(g, x)  # logits = embeddings (nur 2D, aber konsistent)
        if return_embeddings:
            return x, x  # (logits, embeddings)
        return x
...
def robust_load_elliptic_dataset(
    ...
    try:
        ...
        # Node-IDs (entsprechen DF.index-Reihenfolge = txId-Reihenfolge)
...
def export_gcn_embeddings_to_parquet(
    model,
    g,
    features,
    tx_ids,
    ...
    """
    Exportiert Node-Embeddings des trainierten GCN als Parquet-Datei
    in das Artifact-Verzeichnis, analog zu Graphwave / node2vec.

    - model:   Net1/2/3/4 mit support für return_embeddings=True
    - g:       DGLGraph (auf richtigem device)
    - features:Feature-Tensor (N × F) auf richtigem device
    - tx_ids:  Array/List der txId-Strings in Node-Reihenfolge
    - artifact_dir: Pfad zum jeweiligen Run-Ordner (ARTIFACT_ROOT / artifact_dir_name)
    """
    model.eval()
    with th.no_grad():
    ...
    # tx_ids nach numpy-Array casten
    if isinstance(tx_ids, th.Tensor):
        tx_ids_np = tx_ids.detach().cpu().numpy().astype(str)
    else:
        tx_ids_np = np.asarray(tx_ids, dtype=str)

    # Spaltennamen für die Embeddings
    dim = z_np.shape[1]
    emb_cols = [f"gcn_emb_{i}" for i in range(dim)]

    df = pd.DataFrame(z_np, columns=emb_cols)
    df.insert(0, "txId", tx_ids_np)

    # Embeddings NICHT im elliptic-Ordner speichern, sondern direkt in artifacts
    global_emb_dir = os.path.join(".", "artifacts")
    os.makedirs(global_emb_dir, exist_ok=True)
    ...
    fname = f"gcn_embeddings{suffix}.parquet"
    out_path = os.path.join(global_emb_dir, fname)

    df.to_parquet(out_path, index=False)
    print(f"GCN-Embeddings global gespeichert: {out_path}")
...
def run_for_artifact(
    ...
    # Nach dem finalen Training: Embeddings exportieren
    export_gcn_embeddings_to_parquet(
        model=net_final,
        g=g,
    ...
    json_path = os.path.join(
        RESULTS_DIR, f"elliptic_GCN_{artifact_dir_name}{suffix}.json"
    )
    ...
    df_perm_path = (
        out_base / f"elliptic_GCN_{artifact_tag}{suffix}__perm.parquet"
    )
    df_perm.to_parquet(df_perm_path, index=False)

    # 2) Hyperopt-Trials als joblib
    trials_path = (
        out_base / f"elliptic_GCN_{artifact_tag}{suffix}__trials.joblib"
    )
    ...
    bundle_path = (
        out_base / f"elliptic_GCN_{artifact_tag}{suffix}__bundle.joblib"
    )
...
def main():
    ...
    ap.add_argument(
        ...
        type=int,
        default=None,
        help="Percentage of edges removed (z.B. 25, 50, 75). "
        "Wenn None: voller Graph.",
    )
    ...
    if args.artifact or ARTIFACT_DIR_NAME_DEFAULT:
        ...
        run_for_artifact(name, drop_rate=args.drop_rate, variant=args.variant)
        return
    ...
    for name in sorted(os.listdir(root)):
        ...
        if args.skip_existing:
            suffix, _ = get_edge_suffix_and_path(args.drop_rate, args.variant)
            out = os.path.join(
                RESULTS_DIR, f"elliptic_GCN_{name}{suffix}.json"
            )
    ...
    for name in candidates:
        try:
            path = run_for_artifact(
                name, drop_rate=args.drop_rate, variant=args.variant
            )
