# -*- coding: utf-8 -*-
"""
Graph Attention Network - EndToEnd

- Validation threshold tuning τ* (Balanced Accuracy by default) with reporting at both 0.5 and τ*
- Best-epoch transfer: final retraining on (Train + Validation) exactly up to the best epoch found during validation
    → prevents overfitting/underfitting and avoids AUC/Balanced-Accuracy drops in the final test
- Cleaner JSON outputs and extras, including τ*, best_epoch, and the threshold metric
- Batch execution across multiple artifact directories (analogous to GCN)
- Additional artifacts: Hyperopt trials, feature importance, and a bundled export (joblib/parquet)
"""
...
class GATLayer(nn.Module):
    """
    Hidden: concat heads
    Output: 1 head, mean (concat=False)
    """

    def __init__(self, in_feats, out_feats, params, concat=True):
        super().__init__()
        self.params = params
        self.num_heads = int(params.get("num_heads", 1))
        self.concat = bool(concat)
        self.post_drop = nn.Dropout(float(params.get("dropout_rate", 0.0)))
...
class Net1(nn.Module):
    def __init__(self, params, in_feats):
        super().__init__()
        params_out = {**params, "num_heads": 1}
        self.layer1 = GATLayer(in_feats, 2, params_out, concat=False)
...
class Net2(nn.Module):
    def __init__(self, params, in_feats):
        ...
        self.layer1 = GATLayer(in_feats, h2, params, concat=True)
        params_out = {**params, "num_heads": 1}
        in2 = h2 * int(params["num_heads"])
        self.layer2 = GATLayer(in2, 2, params_out, concat=False)
...
class Net3(nn.Module):
    def __init__(self, params, in_feats):
        ...
        self.layer1 = GATLayer(in_feats, h2, params, concat=True)
        in2 = h2 * int(params["num_heads"])
        self.layer2 = GATLayer(in2, h3, params, concat=True)
        params_out = {**params, "num_heads": 1}
        in3 = h3 * int(params["num_heads"])
        self.layer3 = GATLayer(in3, 2, params_out, concat=False)
...
class Net4(nn.Module):
    def __init__(self, params, in_feats):
        ...
        self.layer2 = GATLayer(in2, h3, params, concat=True)
        in3 = h3 * int(params["num_heads"])
        self.layer3 = GATLayer(in3, h4, params, concat=True)
        params_out = {**params, "num_heads": 1}
        in4 = h4 * int(params["num_heads"])
        self.layer4 = GATLayer(in4, 2, params_out, concat=False)
...
def robust_load_elliptic_dataset(params, artifact_dir: str):
    def _rp(fname):
...
def run_for_artifact(
    ...
    def objective(params):
        ...
        try:
            g, features, labels, train_mask, test_mask, val_mask, df_edge = (
                robust_load_elliptic_dataset(params, artifact_dir)
            )
        except Exception as e:
    ...
    try:
        g, features, labels, train_mask, test_mask, val_mask, df_edge = (
            robust_load_elliptic_dataset(best_params, artifact_dir)
        )
    except Exception as e:
    ...
    json_path = out_base / f"elliptic_GAT_{artifact_dir_name}.json"
    ...
    df_perm_path = out_base / f"elliptic_GAT_{artifact_tag}__perm.parquet"
    df_perm.to_parquet(df_perm_path, index=False)

    # 2) Hyperopt-Trials als joblib
    trials_path = out_base / f"elliptic_GAT_{artifact_tag}__trials.joblib"
    ...
    bundle_path = out_base / f"elliptic_GAT_{artifact_tag}__bundle.joblib"
...
def main():
    ...
    if args.artifact:
        # explizit ein Artefakt
        run_for_artifact(args.artifact)
        return

    if ARTIFACT_DIR_NAME_DEFAULT and not args.folder and not args.pattern:
        # Fallback: Default-Artefakt, wenn NICHT im Batchmodus
        run_for_artifact(ARTIFACT_DIR_NAME_DEFAULT)
        return
    ...
    for name in sorted(os.listdir(root)):
        ...
        if args.skip_existing:
            out = os.path.join(RESULTS_DIR, f"elliptic_GAT_{name}.json")
    ...
    for name in candidates:
        try:
            path = run_for_artifact(name, artifact_root=root)
